...
- [ ] Skip CI builds and Vercel deploys for packages that didn't change
  - Use a `changes` job with `dorny/paths-filter` (and a dynamic matrix for apps/templates) so
    `build-web` / `build-apps` / `build-templates` only run when their paths (or `bun.lock` /
    root `package.json`) change; keep root `fmt:check` always-on, and treat `content/**` as part of
    `web`
